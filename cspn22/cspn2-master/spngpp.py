import numpy as np
from learnspnn import Mixture, Separator, GPMixture, Color
import gc
import torch
import gpytorch
...
class Sum:
    ...
    def forward(self, x_pred, label_y, **kwargs):
        ...
        if len(self.children) == 2:
            ...
            # mu_x1 = np.empty((len(x_pred), len(self.weights)))
            # mu_x2 = np.empty((len(x_pred), len(self.weights)))
            # mu_x1[:, 0] = mu_x[:, 0]
            # mu_x2[:, 1] = mu_x[:, 1]
            # index_max = np.argmax(self.weights)
            ...
            # return_mu, return_cov = mu_x[:,index_max].reshape(-1, 1), co_x @_wei
        ...
        if len(self.children) == 3:
            ...
            # mu_x1 = np.empty((len(x_pred), len(self.weights)))
            # mu_x2 = np.empty((len(x_pred), len(self.weights)))
            # mu_x3 = np.empty((len(x_pred), len(self.weights)))
            # mu_x1[:, 0] = mu_x[:, 0]
            # mu_x2[:, 1] = mu_x[:, 1]
            # mu_x3[:, 2] = mu_x[:, 2]
            # index_max = np.argmax(self.weights)
...
class Split:
    ...
    def forward(self, x_pred, label_y, **kwargs):
        ...
        if not smudge:
            mu_x[left_idx], co_x[left_idx], mll_x[left_idx] = left.forward(x_pred[left_idx], label_y[left_idx],
                                                                           **kwargs)
            mu_x[right_idx], co_x[right_idx], mll_x[right_idx] = right.forward(x_pred[right_idx], label_y[right_idx],
                                                                               **kwargs)
        else:
    ...
    def true_variance(self, x_pred, label_y, mu, **kwargs):
        ...
        if not smudge:
            mu_x[left_idx], co_x[left_idx], mll_x[left_idx] = left.true_variance(x_pred[left_idx], label_y[left_idx],
                                                                                 mu, **kwargs)
            mu_x[right_idx], co_x[right_idx], mll_x[right_idx] = right.true_variance(x_pred[right_idx],
                                                                                     label_y[right_idx], mu, **kwargs)
        else:
            inter_idx = np.intersect1d(left_idx, right_idx)
...
class GP:
    ...
    def init1(self, **kwargs):
        ...
        # noise_constraint=gpytorch.constraints.LessThan(1e-2))
        # (noise_prior=gpytorch.priors.NormalPrior(3, 20))
        self.model = ExactGPModel1(x=self.x, y=self.y, likelihood=self.likelihood, type=self.type).to(
            self.device)  # .cuda()
        self.optimizer = Adam([{'params': self.model.parameters()}], lr=lr)
        self.model.train()
        self.likelihood.train()
...
def structure(root_region, **kwargs):
    ...
    gp_types = dict.get(kwargs, 'gp_types', ['rbf'])
    ...
    while len(to_process):
        ...
        if type(gro) is Mixture:
            ...
            to_process.extend(zip(gro.children,
                                  sto.children))  # children of gro and sto are grouped together and put into root_region
            ...
            for gp_type in gp_types:
                ...
                if not dict.get(gps, key1):
                    ...
                    pass
                sto.children.extend([ gps1[key1], gps2[key2]])

            _cn = len(sto.children)
            sto.weights = np.ones(_cn) / _cn

    return root, list(gps1.values()), list(gps2.values())  # modified
