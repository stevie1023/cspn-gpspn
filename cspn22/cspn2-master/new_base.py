...
def leaf_predict_mode(node, data):
    ...
    return node.predictor.predict(data)
...
def supervised_leaf_top_down_mpe(node, input_vals, data=None, lls_per_node=None):
    ...
    return get_mpe_top_down_leaf(node, input_vals, data=data, mode=mode_data[:, 0])
...
class Sum(Node):
    def __init__(self, weights=None, children=None,type = None):
        Node.__init__(self)
        self.type = Sum
        if weights is None:
            weights = []
...
def eval_spn_bottom_up(node, eval_functions, all_results=None, debug=False, **args):
    ...
    for n in nodes:
        ...
        result_=[]
        if isinstance(n, Leaf):
            result = n.predictor.predict(args['data'])
        else:
            func = n.__class__._eval_func[-1]
            # if not hasattr(n.children[0], 'predictor'):
            if n.children[0].scope != []:
                len_children = len(n.children)
                if len_tmp_children_list < len_children:
                    tmp_children_list.extend([None] * len_children)
                    len_tmp_children_list = len(tmp_children_list)
...
def eval_spn_top_down(root, eval_functions, all_results=None, parent_result=None, **args):
    ...
    for layer in reversed(get_topological_order_layers(root)):
        for n in layer:
            if isinstance(n,Leaf):
                param = all_results[n]
                result = supervised_leaf_top_down_mpe(n, param, **args)
