...
def build_bins(**kwargs):
    ...
    while len(to_process):
        ...
        split1_mean=np.mean(node_splits)
        split2_mean = np.mean(node_splits2)
        split3_mean = np.mean(node_splits3)


        # q1 = np.percentile(X, 25, axis=0)
        # q3 = np.percentile(X, 75, axis=0)

        # sm = (q3[d]-q1[d]) / (q1[d] + q3[d])
        # sm2 = (q3[d2]-q1[d2]) / (q1[d2] + q3[d2])
        # sm3 = (q3[d3]-q1[d3]) / (q1[d3] + q3[d3])
        ...
        # split71 = 0.5 * (v1 * r[d] + mins[d]) + 0.5 * np.median(node_splits)
        # split72 = 0.5 * (v2 * r[d2] + mins[d2]) + 0.5 * np.median(node_splits2)
        # split73 = 0.5 * (v3 * r[d3] + mins[d3]) + 0.5 * np.median(node_splits3)

        # split81 = 0.5 * (v1 * sm + mins[d]) + 0.5 * split11
        # split82 = 0.5 * (v2 * sm2 + mins[d2]) + 0.5 * split12
        # split83 = 0.5 * (v3 * sm3 + mins[d3]) + 0.5 * split13
        node_splits_all = [np.median(node_splits), np.median(node_splits2), np.median(node_splits3)]
        # print('median:',node_splits_all_print)
        # node_splits_all = [split71,split72,split73]
        ...
        if reduce_branching and node.depth >= 1:
            ...
            # node_splits_all = [np.mean(node_splits),np.mean(node_splits2)]
            # node_splits_all = [split71,split72]
        ...
        for split in node_splits_all:# again operate in splits in one dimension
            if m == 0:
                ...
                create_right[d[m]] = split != node_splits[-1] #create left(right)nodes for splits other than the first/last split
            # no left nodes for the first split and no right nodes for the last split
            print('node2',node_splits2)

            if m==1:
                create_left = create.copy()
                create_right = create.copy()
                create_left[d[m]] = split != node_splits2[0]
                create_right[d[m]] = split != node_splits2[-1]

            if m==2:
                create_left = create.copy()
                create_right = create.copy()
                create_left[d[m]] = split != node_splits3[0]
                create_right[d[m]] = split != node_splits3[-1]
