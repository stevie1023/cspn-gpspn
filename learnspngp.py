    ...
    while len(to_process):
        ...
        for split in node_splits_all:  # again operate in splits in one dimension
            if m == 0:
                ...
                create_right[d[m]] = split != node_splits_all[0]  # create left(right)nodes for splits other than the first/last split
            ...
            idx_right = query(X, new_mins, node.maxs,
                              skipleft=True)  # return the first index of data that satisfies certain requirements
